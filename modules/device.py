...
def enable_tf32():
    if torch.cuda.is_available():
        ...
        # tf32가 가능한 device의 경우 정밀도 조정 가능
...
def autocast(disable=False):
    ...
    if dtype == torch.float32 or shared.cmd_opts.precision == "full":
        return contextlib.nullcontext() #빈 컨텍스트를 제공
...
def test_for_nans(x, where):
    ...
    if where == "unet":
        message = "Unet에서 NaN값 tensor 생성됨"
    ...
    message += " nan값 확인 기능을 끌려면 --disable-nan-check commandline argument를 command에 입력하세요."
